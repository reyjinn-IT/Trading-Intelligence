"""Deadman Switch implementation to prevent stuck orders and runaway execution."""
import time
import threading
import logging
...
class DeadmanSwitch:
    ...
    def register_cancel_callback(self, cb: Callable[[], None]) -> None:
        """Register a callback to cancel open orders upon timeout trigger."""
        self.cancel_callbacks.append(cb)

    def arm(self) -> None:
        """Arm the deadman switch to start monitoring heartbeats."""
        with self._lock:
            self.last_heartbeat = time.time()
            self.is_armed = True
            self.is_triggered = False
    ...
    def heartbeat(self) -> None:
        """Send a heartbeat to keep the switch from tripping."""
        with self._lock:
            self.last_heartbeat = time.time()
    ...
    def reset(self) -> None:
        """Reset the switch after being triggered."""
        with self._lock:
            self.last_heartbeat = time.time()
            self.is_triggered = False
            self.is_armed = True
    ...
    def get_status(self) -> dict:
        """Return the current state of the deadman switch."""
    ...
    def _trigger_emergency(self) -> None:
        """Execute emergency procedure: cancel open orders and lock trading."""
        with self._lock:
            if self.is_triggered:
                return
